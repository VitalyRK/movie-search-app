...
const nextConfig = {
  ...
  async rewrites() {
    return [
      ...
      {
        source: '/movies/:page/',
        destination: `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&include_adult=false&include_video=false&language=en-US&page=:page&sort_by=popularity.desc/`,
      },
      {
        source: '/details/:id/',
